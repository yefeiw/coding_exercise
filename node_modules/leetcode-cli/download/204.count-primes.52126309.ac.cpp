 ...
 * divisible by some number p, then n = p × q and since p &le; q, we could
 * derive that p &le; &radic;n.
 * 
 ...
 * Eratosthenes Animation" by SKopp is licensed under CC BY 2.0.
 * 
 * 
 * We start off with a table of n numbers. Let's look at the first number, 2.
 * We know all multiples of 2 must not be primes, so we mark them off as
 * non-primes. Then we look at the next number, 3. Similarly, all multiples of
 * 3 such as 3 × 2 = 6, 3 × 3 = 9, ... must not be primes, so we mark them off
 * as well. Now we look at the next number, 4, which was already marked off.
 * What does this tell you? Should you mark off all multiples of 4 as
 * well?
 * 
 * 4 is not a prime because it is divisible by 2, which means all multiples of
 * 4 must also be divisible by 2 and were already marked off. So we can skip 4
 * immediately and go to the next number, 5. Now, all multiples of 5 such as 5
 * × 2 = 10, 5 × 3 = 15, 5 × 4 = 20, 5 × 5 = 25, ... can be marked off. There
 * is a slight optimization here, we do not need to start from 5 × 2 = 10.
 * Where should we start marking off?
 * 
 * In fact, we can mark off multiples of 5 starting at 5 × 5 = 25, because 5 ×
 ...
 * p: p2 + p, p2 + 2p, ... Now what should be the terminating loop
 * condition?
 * 
 ...
 * 
 * Yes, the terminating loop condition can be p < &radic;n, as all non-primes
 * &ge; &radic;n must have already been marked off. When the loop terminates,
 ...
 * complexity is O(n log log n). For the more mathematically inclined readers,
